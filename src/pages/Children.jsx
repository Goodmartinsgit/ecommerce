...
const Children = () => {
  ...
  useEffect(() => {
    if (productData && productData.length > 0) {
      ...
      const childrensCloth = productData.filter((item) =>
        item?.category?.name?.toLowerCase().includes("child") ||
        item?.category?.name?.toLowerCase().includes("kid")
      );
      setChildrensCloth(childrensCloth);
    }
  }, [productData]);
