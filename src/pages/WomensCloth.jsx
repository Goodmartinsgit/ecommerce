...
const WomensCloth = () => {
  ...
  useEffect(() => {
    if (productData && productData.length > 0) {
      ...
      const womensCloth = productData.filter((item) =>
        item?.category?.name?.toLowerCase().includes("women") ||
        item?.category?.name?.toLowerCase() === "female"
      );
      setWomensCloth(womensCloth);
    }
  }, [productData]);
      ...
      <div className="bg-white min-h-screen py-8">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-center text-primary mt-2 text-lg">
            Explore our complete collection of womens cloth.
          </p>

          {womensCloth.length === 0 ? (
            <div className="text-center mt-16">
              <p className="text-gray-600 text-lg">No womens cloth found.</p>
            </div>
          ) : (
            <>
              <p className="text-center text-gray-600 mt-4">
                Showing {womensCloth.length} product
                {womensCloth.length !== 1 ? "s" : ""}
              </p>
